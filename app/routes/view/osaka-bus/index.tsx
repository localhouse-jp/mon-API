...
export default function OsakaBusIndex() {
  // 重複を除いたバス停一覧を取得
  const stopNames = Array.from(
    new Set(
      osakaBusRoutes.flatMap(route => route.stops.map(stop => stop.stopName))
    )
  )
  ...
  return (
    ...
    >
      <h1>大阪バス時刻表</h1>
      <p><a href="/">← ホームに戻る</a> | <a href="/view/osaka-bus/calendar">運行カレンダーを見る</a></p>

      <h2>バス停一覧</h2>
      <div className="stops-list">
        {stopNames.map(stopName => (
          <a
            href={`/view/osaka-bus/stop/${encodeURIComponent(stopName)}`}
            className="stop-link"
            key={stopName}
          >
            {stopName}
      ...
      <h2>路線情報</h2>
      {osakaBusRoutes.map((route, index) => (
